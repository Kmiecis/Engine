...
namespace Engine
{
    static GLenum ShaderTypeFromString(const std::string& type)
    {
        ...
        NG_CORE_ASSERT(false, "Unknown shader type!");
        return 0;
    }
    ...
    std::string OpenGLShader::ReadFile(const std::string& filepath) const
    {
        ...
        if (in)
        {
            ...
            {
                NG_CORE_ERROR("Could not read from file '{0}'", filepath);
            }
        }
        else
        {
            NG_CORE_ERROR("Could not open file '{0}'", filepath);
        }
    ...
    std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
    {
        ...
        while (pos != std::string::npos)
        {
            ...
            NG_CORE_ASSERT(eol != std::string::npos, "Syntax error");
            size_t begin = pos + typeTokenLength + 1; //Start of shader type name (after "#type " keyword)
            std::string type = source.substr(begin, eol - begin);
            NG_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");

            size_t nextLinePos = source.find_first_not_of("\r\n", eol); //Start of shader code after shader type declaration line
            NG_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
    ...
    void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& sources)
    {
        ...
        NG_CORE_ASSERT(sources.size() <= 2, "We only support 2 shaders for now");
